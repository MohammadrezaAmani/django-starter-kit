...
def payment_callback(request):
    """Handle bank callback after payment."""
    tracking_code = request.GET.get(
        settings.AZ_IRANIAN_BANK_GATEWAYS["TRACKING_CODE_QUERY_PARAM"], None
    )
    ...
    try:
        ...
        payment = Payment.objects.get(transaction_id=tracking_code)  # type: ignore
    except (bank_models.Bank.DoesNotExist, Payment.DoesNotExist):  # type: ignore
        log_user_action(
            request=request,
            action_type=AuditLog.ActionType.SYSTEM,
            status=AuditLog.Status.FAILED,
    ...
    redirect_url = (
        f"/payment/{'success' if is_success else 'failed'}/{payment.transaction_id}/"
    )
...
def go_to_gateway(request):
    """Initiate payment and render gateway form."""
